...
fn view_dialog(mut dialog: Signal<RoutineDialog>, search_term: &str) -> Element {
    let mut is_loading = use_signal(|| false);

    macro_rules! close_dialog {
        ($dialog:expr, $search_term:expr) => {{
    ...
    let search_term_for_save = search_term.to_string();
    let save = move |_| {
        let search_term = search_term_for_save.to_string();
        async move {
            let mut saved = false;
            is_loading! {
            ...
            if saved {
                close_dialog!(dialog, search_term);
            }
        }
    };
    let search_term_for_delete = search_term.to_string();
    let delete = move |_| {
        let search_term = search_term_for_delete.to_string();
        async move {
    ...
    match &*dialog.read() {
        ...
        RoutineDialog::Options(routine) => {
            ...
            let search_term_for_archive_option = search_term.to_string();
            rsx! {
                OptionsMenu {
                    options: vec![
                        rsx! {
                            MenuOption {
                                ...
                                onclick: move |_| {
                                    let search_term = search_term_for_archive_option.clone();
                                    async move {
                                        match DOMAIN_SERVICE
                                            .read()
        ...
        RoutineDialog::Delete(routine) => rsx! {
            DeleteConfirmationDialog {
                ...
                cancel_event: close!(search_term),
