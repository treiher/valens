...
use crate::{
    ...
    ui::{
        element::{
            ...
            LoadingPage, NoConnection, NoWrap, Table, value_or_dash,
        },
...
pub fn Training(add: bool) -> Element {
    ...
    let current_interval =
        use_signal(|| domain::init_interval(&dates.read(), domain::DefaultInterval::_1M));
    ...
    let show_add_dialog = move || async move {
        ...
        navigator().replace_preserving_scroll(Route::Training { add: true });
    };
    ...
    match (
        ...
        (CacheState::Ready(training_sessions), CacheState::Ready(routines), Some(Ok(settings))) => {
            rsx! {
        ...
        (CacheState::Error(err), _, _) | (_, CacheState::Error(err), _) => {
            rsx! { ErrorMessage { message: err } }
        }
        (_, _, Some(Err(err))) => {
            rsx! { ErrorMessage { message: err } }
        }
        (CacheState::Loading, _, _) | (_, CacheState::Loading, _) | (_, _, None) => {
            rsx! { LoadingPage {} }
        }
    }
}
...
pub fn view_dialog(
    ...
    let close_dialog = move || {
        dialog.set(TrainingDialog::None);
