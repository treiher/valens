...
pub fn BodyFat(add: bool) -> Element {
    ...
    let dates = use_memo(move || {
        if let CacheState::Ready(body_fat) = &*cache.body_fat.read() {
            body_fat.iter().map(|bf| bf.date).collect::<Vec<_>>()
        } else {
            vec![]
        }
    });
    ...
    let show_add_dialog = move || async move {
        ...
        navigator().replace_preserving_scroll(Route::BodyFat { add: true });
    };
...
fn table(
    ...
    let body = body_fat
        ...
        .map(|bf| {
            ...
            row.push(rsx! {
                ItemOptionsButton { on_click: move |_| { *dialog.write() = BodyFatDialog::Options(bf.clone()); } }
            });
            row
        })
        .collect::<Vec<_>>();
...
fn view_dialog(mut dialog: Signal<BodyFatDialog>, sex: domain::Sex) -> Element {
    ...
    let mut close_dialog = move || {
        dialog.set(BodyFatDialog::None);
        navigator().replace_preserving_scroll(Route::BodyFat { add: false });
    };
            ...
            SaveDialog {
                ...
                InputField {
                    ...
                    on_input: move |event: FormEvent| {
                        ...
                        async move {
                            let validated_date = DOMAIN_SERVICE().validate_body_fat_date(&input).await.map_err(|err| err.to_string());
                            match &mut *dialog.write() {
