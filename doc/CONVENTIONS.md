...
- A claim about a value belongs to `domain` or to an extracted helper, a claim about what a component renders for a given state to a render test, and a claim that needs a browser to an end-to-end test.
- A claim needs a browser where it depends on real events, on the document as the browser builds it, or on a browser API, as in an interaction, router state, a reload, offline behavior, a backend round trip, layout, timing, or the clipboard.
- Where both a render test and an end-to-end test could hold an assertion, the render test wins, unless reaching the state needs more scaffolding than the state is worth.
- A feature a user reaches through the interface keeps end-to-end coverage of that path, however much render coverage it has. What moves out are the variations on the state so reached: the empty and error states, the filter combinations, the boundary data, and the variants of a user property such as the sex.
- Render tests render a component to HTML on the host with the helpers in [`test_render.rs`](../crates/web-app-dioxus/src/test_render.rs), and locate elements by test-id or by the text the user sees, never by class name or by structural position from the root. Each test asserts the part of the output it is about: a chart through the branch it renders and the controls and tables around it, never through the generated SVG.
- End-to-end tests locate elements by test-id, not by styling. Test-ids are never used in these tests directly; they are encapsulated in the page objects under [`tests/e2e/pages`](../tests/e2e/pages), which are introduced or extended as needed. Render tests use test-ids directly.
- Tests assert element state through data attributes, not through CSS classes. A state that tests need to observe, such as the selected theme, the drop state during a drag or the severity of a log entry, is rendered as a `data-*` attribute, so that restyling does not break the tests.
